...
- [ ] Configurar `application.yaml` (datasource, JPA, Flyway, OpenAPI, Actuator)
- [ ] `docker-compose.yml` com PostgreSQL
- [ ] Entidades: `Client`, `User`, `SupportTeam`, `Ticket` + enums
...
- [ ] Mappers (**MapStruct**)
- [ ] CRUD de Client, User, SupportTeam
- [ ] `@RestControllerAdvice` para erros padronizados
- [ ] OpenAPI exposto e revisado

## Fase 3 — Criação de chamado com padrões

- [ ] **Builder** na entidade `Ticket`
- [ ] **Factory Method** para tipos de chamado
- [ ] **Facade** de criação (`TicketFacade`)
- [ ] `POST /tickets` ponta a ponta
